...
Gdy werdykt to **AI slop**, cała karta dostaje klasę `li-ai-slop-blurred`. Rozmycie jest mocne: klasa `.lais-paint-blur` z `filter: blur(20px)` na boksach, które naprawdę malują (wrapper `display: contents` jest pomijany, obrazek dostaje filtr, nie dziecko), oraz welon `.lais-blur` (`backdrop-filter: blur(24px)` i szron). Gdy sam korzeń karty jest `display: contents`, welon ląduje w tych boksach, a nie na zerowym `inset: 0`. Sam `backdrop-filter` na białej karcie LinkedIn bywa niewidoczny, bo przodek z `transform` albo `contain` ucina tło. Odznaka zostaje nad welonem i jest ostra. Ludzki i Mieszany nie są rozmywane. Opcja „Rozmywaj posty AI slop” (Blur AI Slop posts) jest domyślnie włączona. „Pokaż” na odznace odsłania jedną kartę. To samo dotyczy kart recent-activity, które content script już oznacza.
